...
def make_annot_files(args, bed_for_annot):
    ...
    len_orig = len(df_int)
    df_int = df_int.drop_duplicates(subset=['BP'])
    n_dropped = len_orig - len(df_int)
    if n_dropped > 0:
        print(f"Dropped {n_dropped} duplicate BP positions from annot (before: {len_orig}, after: {len(df_int)})")
